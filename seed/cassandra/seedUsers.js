...
async function executeOneAtATime(info) {
  const query = `INSERT INTO users (userid, username, profileurl, createdat, updatedat) VALUES (?, ?, ?, ?, ?)`;
  ...
  while (info.counter++ < info.totalLength) {
    ...
    const params = [info.counter, faker.name.firstName(), `https://i.pravatar.cc/150?img=${faker.random.number({min: 1, max: 70})}`, createdat, updateddat];
